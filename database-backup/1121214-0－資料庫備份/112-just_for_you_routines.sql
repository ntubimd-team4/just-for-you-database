...
--
-- Temporary view structure for view `original_v_recommend_record`
--

DROP TABLE IF EXISTS `original_v_recommend_record`;
/*!50001 DROP VIEW IF EXISTS `original_v_recommend_record`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `original_v_recommend_record` AS SELECT 
 1 AS `id`,
 1 AS `s_id`,
 1 AS `user_id`,
 1 AS `establish_time`,
...
--
-- Final view structure for view `original_v_recommend_record`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`ntubimd112404`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `v_recommend_record` AS select uuid() AS `id`,any_value(`s`.`s_id`) AS `s_id`,any_value(`s`.`user_id`) AS `user_id`,any_value(`s`.`establish_time`) AS `establish_time`,any_value(`mr`.`r_id`) AS `r_id`,`m`.`song` AS `song`,any_value(`m`.`m_id`) AS `m_id`,any_value(`m`.`link`) AS `link`,any_value(`m`.`thumbnails`) AS `thumbnails`,any_value(`me`.`emotion_tag`) AS `emotion_tag`,any_value(`c`.`description`) AS `description`,any_value(`mr`.`collection`) AS `collection` from ((((`summary_record` `s` left join `music_recommend` `mr` on((`s`.`s_id` = `mr`.`s_id`))) left join `music_emotion` `me` on((`me`.`id` = `mr`.`music_emo_id`))) left join `music` `m` on((`me`.`m_id` = `m`.`m_id`))) left join `codelist` `c` on((`me`.`emotion_tag` = `c`.`value`))) where (`c`.`code_name` = 'emotion_tag') group by `s`.`user_id`,`m`.`song` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
